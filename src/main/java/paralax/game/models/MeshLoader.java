...
import paralax.game.utils.Lists;
...
public class MeshLoader {
	...
	public static Mesh[] load(String resourcePath, int flags) throws Exception {
		 ...
		 String fileName = Class.class.getResource("/models/" + resourcePath + ".obj").getFile();
	        File file = new File(fileName);
	        // Assimp will be able to find the corresponding mtl file if we call aiImportFile this way.
	AIScene aiScene = aiImportFile(file.getAbsolutePath(), flags);
		...
		for (int i = 0; i < numMaterials; i++) {
			...
			processMaterial(aiMaterial, materials, "/textures/" + resourcePath);
		}
